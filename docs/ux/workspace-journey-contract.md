...
阅读保存和删除带基准 revision，旧版本不会覆盖云端新版。409 后自动拉取并协调一次；
再次失败保留本地修改并提供 Retry，避免无限重试。本地 journal 保存已确认的内容基准；
内容比较忽略属性/列表顺序和笔记更新时间，打开时间取较新值、当前设备的选中位置保留，
两者不参与内容冲突。不同笔记或标题的独立修改自动合并；正文或 anchor 删除影响笔记关联时保守处理。

真正重叠的修改暂停该阅读上传，保持一条阅读并显示 Review sync changes。其他阅读仍可同步。
待处理的本地内容与基准保存在当前设备，刷新后恢复检查；未决定前不更新该阅读的同步基准。
恢复界面展示当前本地及已读取的云端差异，可使用任一方的冲突修改；独立修改仍合并保留。
笔记/成果冲突可在原阅读内保留两个版本。只有明确选择另存阅读，才重新编号并创建新 session；
不自动创建 `(conflict copy)`。此处的待处理本地版本不是跨设备备份，浏览器存储失败会提示保持页面打开。

旧版 journal 缺少基准时，相同内容可直接收敛，不同内容须复核，不能推断修改归属。
删除与内容编辑冲突需显式选择；远端已删除的本地改动只能明确恢复为新阅读，不复活原地址。
已有冲突副本不按名称自动删除。
