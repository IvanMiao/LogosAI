...
export function copyReadingSession(session: ReadingSessionSnapshot): ReadingSessionSnapshot {
  ...
  return {
    document: { ...session.document, id: documentId,
      title: `${session.document.title.slice(0, 140)} (recovered)`, updatedAt: now },
